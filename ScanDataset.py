...
class ScanDataset(Seq2SeqDataset):
    def __init__(
        ...
        split: ScanSplit,
        train: bool = True,
        split_variation=None,
    ):
        super().__init__(input_lang, output_lang, train)

        # Clone SCAN repo if not already cloned
        if not os.path.exists("SCAN"):
            Repo.clone_from("https://github.com/brendenlake/SCAN.git", "SCAN")
    ...
    def _get_data(self, split: ScanSplit, split_variation=None, train: bool = True):
        ...
        if split == ScanSplit.SIMPLE_SPLIT:
            ...
            if split_variation and split_variation in valid_variations:
                ...
                raise Exception(
                    f"Not a valid split variation. Valid variations are: {valid_variations}"
                )
            else:
                X_train, y_train = self._extract_data_from_file(
                    "tasks_train_simple.txt", split
                )
                X_test, y_test = self._extract_data_from_file(
                    "tasks_test_simple.txt", split
                )
        elif split == ScanSplit.LENGTH_SPLIT:

            X_train, y_train = self._extract_data_from_file(
                "tasks_train_length.txt", split
            )
            X_test, y_test = self._extract_data_from_file(
                "tasks_test_length.txt", split
            )
            ...
            if split_variation in valid_action_seq_len:
                filter_idxs = [
                    i for i, y in enumerate(y_test) if len(y.split()) == split_variation
                ]
                ...
                filter_idxs = [
                    i for i, x in enumerate(X_test) if len(x.split()) == split_variation
                ]
                X_test = [X_test[i] for i in filter_idxs]
                y_test = [y_test[i] for i in filter_idxs]
            ...
            if split_variation and split_variation in valid_variations:
                X_train, y_train = self._extract_data_from_file(
                    f"tasks_train_addprim_{split_variation}.txt", split
                )
                X_test, y_test = self._extract_data_from_file(
                    f"tasks_test_addprim_{split_variation}.txt", split
                )
            else:
                raise Exception(
